...
export const ProductHighlightPanel = memo(function ProductHighlightPanel({ product }: ProductHighlightPanelProps) {
  const verdictStyle = VERDICT_STYLES[product.verdict];
  const specIcons = product.category === "stroller" ? [Scale, Ruler, Baby] : [Scale, Shield, Car];
        ...
        <div className="bg-white/55 backdrop-blur-xl border border-white/25 rounded-[20px] shadow-lg p-6 lg:p-8">
          <div className="text-center mb-4">
            <div className="flex items-center justify-center gap-2 mb-2">
              ...
              <Badge className={`text-[10px] ${verdictStyle.className} border`}>{verdictStyle.label}</Badge>
            </div>
            <h2 className="font-serif text-2xl lg:text-3xl font-semibold text-foreground">{product.name}</h2>
          </div>

          {product.lifestyleTags.length > 0 && (
            <div className="flex flex-wrap justify-center gap-2 mb-4">
              {product.lifestyleTags.map((tag) => (
                <span key={tag} className="px-2.5 py-1 bg-azure-100 text-azure-700 rounded-full text-xs font-medium">
          ...
          <div className="space-y-2 mb-6">
            {product.highlights.map((highlight, index) => (
              <motion.div key={highlight} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: index * 0.1 }} className="flex items-center gap-3 text-muted-foreground">
